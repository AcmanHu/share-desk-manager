  ...
  <div class="m-content">
    <div class="m-wrap">
      <div class="search_box">
        ...
        <div>
          <el-button type="primary" size="small" plain @click="sendMessageBtnAll">一键群发</el-button>
  ...
  methods: {
    ...
    sendMessageBtn: function() {
      ...
      this.$prompt("请输入消息内容", "提示", {
        ...
        .then(({ value }) => {
          var info = "";
          for (let index = 0; index < this.numberPhone.length; index++) {
            if (index === 0) {
              info = this.numberPhone[index];
            } else {
              info = info + "," + this.numberPhone[index];
            }
          }
          this.sendMessage(value, info);
        })
        .catch(() => {
          this.$message({
            type: "info",
    ...
    sendMessageBtnAll: function() {
      this.$prompt("请输入消息内容", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消"
      })
        .then(({ value }) => {
          this.sendMessageAll(value);
        })
        .catch(() => {
          this.$message({
            type: "info",
